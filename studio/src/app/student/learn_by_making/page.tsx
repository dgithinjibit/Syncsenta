...
const RECOMMENDED_COURSES: SubjectCard[] = [
  { label: 'AGI', image: '/images/learning-catalog/ai.png', slug: 'ai' },
...
function LearningProgress() {
  const steps = ['Level', 'Sub-Level', 'Grade', 'Make']
  return (
      ...
      {steps.map((step, index) => {
        ...
        return (
          <li key={step} className="relative flex min-w-0 items-center sm:block">
            {index > 0 && <span className="absolute left-0 top-5 hidden h-1 w-full -translate-x-1/2 bg-emerald-500 sm:block" aria-hidden />}
            <span className={`relative z-10 flex h-10 w-10 shrink-0 items-center justify-center rounded-full border-4 text-sm font-bold shadow-sm ${isCurrent ? 'border-orange-200 bg-orange-500 text-white' : 'border-white bg-emerald-500 text-white'}`}>
              {isCurrent ? '4' : <Check className="h-5 w-5" strokeWidth={3} />}
            </span>
            <span className="ml-3 text-sm font-semibold text-teal-700 sm:ml-0 sm:mt-2 sm:block">{step}</span>
          </li>
        )
      })}
    </ol>
...
function ImageCard({ label, image, onClick }: { label: string; image: string; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} className="group relative isolate h-40 overflow-hidden rounded-2xl border border-teal-100 bg-slate-950 text-left shadow-sm transition duration-200 hover:-translate-y-1 hover:border-teal-300 hover:shadow-xl focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-teal-300 sm:h-44">
      ...
      <span className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-3 p-4">
        <span className="text-xl font-extrabold leading-tight tracking-tight text-white drop-shadow-md sm:text-2xl">{label}</span>
...
export default function LearnByMakingPage() {
  ...
  useEffect(() => {
    ...
    if (selectedGrade) {
      ...
      window.localStorage.setItem('learningJourney.grade', selectedGrade)
    }
    if (!selectedGrade) router.replace('/student/journey')
  }, [profile?.grade, router])

  const cards = useMemo(() => [...CORE_SUBJECTS, ...RECOMMENDED_COURSES], [])

  const openSubject = useCallback((card: SubjectCard) => {
    if (!grade) {
      router.push('/student/journey')
      return
    }

    const subject = card.subject ?? card.slug
    if (!subject) {
      setNotice(`${card.label} is not yet available as an interactive activity for this grade.`)
      return
    }

    window.sessionStorage.setItem('learningJourney.subject', card.label)
  ...
  useEffect(() => {
    if (!grade || !requestedSubject) return
    const matching = cards.find((card) => card.label.toLowerCase() === requestedSubject.toLowerCase() || card.subject === requestedSubject || card.slug === requestedSubject)
    if (!matching) return
    const timer = window.setTimeout(() => openSubject(matching), 0)
    return () => window.clearTimeout(timer)
  }, [cards, grade, openSubject, requestedSubject])

  if (!grade) return <div className="min-h-screen bg-teal-400" />
          ...
          <div className="mx-auto max-w-7xl">
            <div className="mb-8 flex flex-col justify-between gap-4 border-b border-teal-100 pb-6 sm:flex-row sm:items-end">
              <div>
                ...
                <p className="mt-2 max-w-2xl text-base leading-7 text-teal-900/70">Choose a subject to open its interactive canvas for <strong>{grade}</strong>. Your attempts, hints, and progress stay connected to this learning context.</p>
              </div>
              <div className="rounded-2xl border border-orange-200 bg-orange-50 px-4 py-3 text-sm font-semibold text-orange-900">Current pathway<br /><span className="text-lg">{grade}</span></div>
            </div>
            <LearningProgress />
            <section aria-labelledby="core-subjects-heading">
              <div className="mb-5 flex items-center gap-2"><Sparkles className="h-5 w-5 text-teal-600" /><h2 id="core-subjects-heading" className="text-2xl font-extrabold text-teal-700 sm:text-3xl">Core Subjects</h2></div>
              <div className="grid grid-cols-1 gap-4 min-[430px]:grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
                {CORE_SUBJECTS.map((card) => <ImageCard key={card.label} {...card} onClick={() => openSubject(card)} />)}
              </div>
            </section>
            <section aria-labelledby="recommended-courses-heading" className="mt-12">
              <h2 id="recommended-courses-heading" className="mb-5 text-2xl font-extrabold text-teal-700 sm:text-3xl">Recommended Courses for You</h2>
              <div className="grid gap-5 md:grid-cols-3">{RECOMMENDED_COURSES.map((course) => <ImageCard key={course.label} {...course} onClick={() => openSubject(course)} />)}</div>
            </section>
            {notice && <div role="status" className="mt-8 rounded-xl border border-teal-200 bg-teal-50 px-4 py-3 text-sm font-medium text-teal-800">{notice}</div>}
          </div>
        </main>
